...
namespace AdventOfCode
{
    class Day4
    {
        ...
        static string regexFecha = @"\[.*\]";
        static string regexDespertar = @"wakes";
        static string regexDormir = @"falls";
        static string regexId = @"#\d+";

        static Regex regLine = new Regex(regexLine);
        static Regex regFecha = new Regex(regexFecha);
        static Regex regDespertar = new Regex(regexDespertar);
        static Regex regDormir = new Regex(regexDormir);
        static Regex regId = new Regex(regexId);

        public enum Accion { Comienzo,Dormir,Despertar}
        public class Suceso
        {
            ...
            public Suceso(string line)
            {
                string f = regFecha.Match(line).Value;
                f = f.Replace("[", "").Replace("]", "");
                fecha = DateTime.Parse(f);

                Accion = (regDespertar.IsMatch(line)) ? Accion.Despertar :
                        (regDormir.IsMatch(line)) ? Accion.Dormir :
                        (regId.IsMatch(line)) ? Accion.Comienzo : Accion.Comienzo;

                try
                {
                    id = int.Parse(regId.Match(line).Value.Replace("#", ""));
                }
                catch { }
            }
        }
        ...
        {
            public int id;
            public int mayorMinutos;
            public int acumulado;
        }

        public static void Part1()
        {
            Regex reg = new Regex(regexLine);
            var matchCollection = reg.Matches(input);
            List<Suceso> sucesos = new List<Suceso>();
            List<Guardia> guardias = new List<Guardia>();
            foreach (Match match in matchCollection)
            {
                sucesos.Add(new Suceso(match.Value));
            }

            sucesos = sucesos.OrderBy(o => o.fecha.Ticks).ToList();

            foreach (Suceso s in sucesos)
            {
                Console.WriteLine($"{s.fecha.ToString()} {s.id} {s.Accion}");
            }


            guardias = new List<Guardia>(sucesos.Where(w => w.id != null).Select(s => s.id.Value).Distinct().Select(i => new Guardia { id = i }));
            int guardiaSeleccionado = 0;
            DateTime ultimoComienzoDormir = DateTime.Now;
            for(int i = 0; i < sucesos.Count; i++)
            {
                Suceso suceso = sucesos[i];
                switch (suceso.Accion)
                {
                    case Accion.Comienzo:
                        {
                            guardiaSeleccionado = suceso.id.Value;
                        }
                        break;
                    case Accion.Dormir:
                        {
                            ultimoComienzoDormir = suceso.fecha;
                        }
                        break;
                    case Accion.Despertar:
                        {
                            int minutes = TimeSpan.FromTicks(suceso.fecha.Ticks - ultimoComienzoDormir.Ticks).Minutes;
                            Guardia guardian = guardias.Find(f => f.id == guardiaSeleccionado);
                            guardian.acumulado += minutes;
                            if (guardian.mayorMinutos < minutes)
                            {
                                guardian.mayorMinutos = minutes;
                            }
                        }
                        break;
            ...
            foreach (Guardia g in guardias.OrderByDescending(o=>o.acumulado))
            {
                Console.WriteLine($"{g.id} {g.acumulado}  {g.mayorMinutos} {g.id* g.mayorMinutos}");
            }
        ...
        public static string input = @"[1518-09-02 00:18] wakes up
